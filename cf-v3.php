...
function cfv3_customizacaoAdmin() {
	...
	function cfv3_remove_menus_editor(){
        ...
        // add_menu_page( 
        // 	__( 'Temas e Banco de Imagem', 'cf_v3' ), 
        // 	__( 'Temas e Banco de Imagem', 'cf_v3' ), 
        // 	'edit_posts', 
        // 	'envato-elements', 
        // 	null, 
        // 	'https://modelo.convertefacil.com.br/wp-content/plugins/envato-elements/assets/navigation.svg', 
        // 	58.6 
        // );
	...
	function cfv3_rename_menus(){
        ...
        // $menu[ 2 ][ 0 ] = __( 'Dashboard Converte Fácil', 'cf_v3' );			// Painel
        // $menu[ 5 ][ 0 ] = __( 'Conteúdo e SEO Inteligente', 'cf_v3' );			// Posts
        // $menu[ 10 ][ 0 ] = __( 'Biblioteca de Arquivos', 'cf_v3' );				// Mídia
        // $menu[ 20 ][ 0 ] = __( 'Páginas do Site e Landing Pages', 'cf_v3' );			// Páginas
        // $menu[ 25 ][ 0 ] = __( 'Moderar Comentários', 'cf_v3' );			// Comentários
        // $menu[ 27 ][ 0 ] = __( 'Formulários de Contato', 'cf_v3' );			// Contato
        // if( isset( $menu[ '58.6' ] ) )
        // 	$menu[ '58.6' ][ 0 ] = __( 'Temas e Banco de Imagem', 'cf_v3' );		// Elements
        // $menu[ 100 ][ 0 ] = __( '', 'cf_v3' );		// Painel
        // $submenu[ 'users.php' ][ 15 ][ 0 ] = __( 'Administrar Meu Perfil', 'cf_v3' );			// Perfil
	...
	function cf_v3_new_users_page() {
		...
		<div class="wrap" id="new-users-page">
			...
			<?php /* ?><h1 class="wp-heading-inline"><?php _e( 'Novos usuários', 'cf_v3' ); ?></h1><?php */ ?>
	...
	function cfv3_menus_edit_style() {
		...
		<style>
			...
			/* caixas de texto do painel 
			.postbox>h3, .metabox-holder .stuffbox>h3, .metabox-holder h2.hndle, .metabox-holder h3.hndle {display: none !important;}
			.accordion-section-title:after, .handlediv, .item-edit, .postbox .handlediv.button-link, .toggle-indicator {display: none !important;} */
	...
	function cfv3_get_widgets() {
		...
		foreach( $widgets as $k => $widget ) :
			...
			add_meta_box( 'cf-widget-' . $k, $widget[ 'title' ], function() use ( $content ) { echo $content; } , 'dashboard', $widget[ 'position' ], 'high' );
		endforeach;
	...
	function cfp_admin_notice() {
		...
		// cfv3_debug( $current_screen->id );
		// cfv3_debug( $current_screen->base );
		// cfv3_debug( $current_screen->parent_base );
		// cfv3_debug( $current_screen->parent_file );
		// cfv3_debug( $current_screen->post_type  );
		...
		// echo '<div class="notice notice-info is-dismissible" style="display: block !important;">
...
// add_action( 'wp_enqueue_scripts', 'cfv3_add_script' );
